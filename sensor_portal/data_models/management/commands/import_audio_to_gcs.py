...
from utils.storage import upload_file, get_bucket, get_blob, get_storage_client
from data_models.models import DataType, Site, Device, Deployment, Project, DataFile, DeviceModel

class Command(BaseCommand):
    help = 'Import audio files from NINA project directory to GCS (fake or real)'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help='GCS bucket name (defaults to GCS_BUCKET_NAME in settings)',
        )
        parser.add_argument(
            '--clean',
            action='store_true',
        ...
        parser.add_argument(
            '--init-gcs',
            action='store_true',
            help='Initialize the fake GCS environment before importing',
        )
    ...
    def handle(self, *args, **options):
        ...
        # Set bucket name
        bucket_name = options['bucket'] or settings.GCS_BUCKET_NAME
        
        # Initialize fake GCS if requested
        if options['init_gcs'] and not options['dry_run']:
            from django.core.management import call_command
            self.stdout.write("Initializing fake GCS environment...")
            call_command('fake_gcs', init=True)
        ...
        self.stdout.write(f"Using GCS base bucket: {bucket_name}")
        ...
        for item in os.listdir(base_dir):
            ...
            # Create a device-specific bucket name
            device_bucket_name = f"{bucket_name}-{device_id}"
            
            # Create the device bucket if it doesn't exist (no-op if it exists)
            device_bucket = client.bucket(device_bucket_name)
            self.stdout.write(f"  Using device bucket: {device_bucket_name}")
                ...
                for file_path in audio_files:
                    ...
                    if not options['dry_run']:
                        ...
                        try:
                            ...
                            try:
                                # Upload to device-specific bucket directly without project/device subdirectories
                                gcs_path = file_name + file_ext
                                
                                if not options['dry_run']:
                                    # Upload the file to device-specific GCS bucket
                                    public_url = upload_file(file_path, device_bucket_name, gcs_path)
                                    self.stdout.write(f"  Uploaded {file_name} to GCS: {public_url}")
                                    stats['uploaded_to_gcs'] += 1
                                else:
                                    public_url = f"(would be uploaded to gs://{device_bucket_name}/{gcs_path})"
                            except Exception as e:
                            ...
                            with connection.cursor() as cursor:
                                ...
                                """, [
                                    ...
                                    file_size,
                                    file_ext,
                                    f"gs://{device_bucket_name}/{gcs_path}",  # Use device bucket path
                                    f"gs://{device_bucket_name}",              # Use device bucket as local path
                                    now,
                                    now,
                                    now,
                                    recording_dt,
                                    False,  # Not in local storage anymore, it's in GCS
                                    False,  # archived
                                    False,  # do_not_remove
                                    public_url  # Set the file_url to the GCS URL
                                ])
        ...
        self.stdout.write(f"  Uploaded to GCS: {stats['uploaded_to_gcs']}")
        if stats['errors'] > 0:
